...
class enrol_token_external extends external_api {
    ...
    public static function get_instance_info_returns() {
        return new external_single_structure(
            array(
                ...
                'token' => new external_value(PARAM_RAW, 'token required for enrolment', VALUE_OPTIONAL),
            )
        );
    }
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'token' => new external_value(PARAM_RAW, 'Token key', VALUE_DEFAULT, ''),
    ...
    public static function enrol_user($courseid, $token = '', $instanceid = 0) {
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_token_enrol($instance);
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_token_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_token')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_token', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_token($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
