...
$PAGE->set_url('/enrol/token/edit.php', array('courseid' => $course->id, 'id' => $instanceid));
...
/** @var enrol_token_plugin $plugin */
$plugin = enrol_get_plugin('token');

if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid' => $course->id, 'enrol' => 'token', 'id' => $instanceid), '*', MUST_EXIST);
}
else {
...
$mform = new enrol_token_edit_form(NULL, array($instance, $plugin, $context));
...
if ($mform->is_cancelled()) {
    ...
    if ($instance->id) {
        ...
        $instance->customint1 = $data->ipthrottlingperiod;
        $instance->customint2 = $data->customint2;
        $instance->customint3 = $data->userthrottlingperiod;
...
$PAGE->set_title(get_string('pluginname', 'enrol_token'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('pluginname', 'enrol_token'));
$mform->display();
echo $OUTPUT->footer();
